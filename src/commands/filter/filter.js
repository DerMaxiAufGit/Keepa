  ...
  async execute(interaction) {
    ...
    if (group === 'links') {
      if (sub === 'add') {
        ...
        try {
          db.prepare('INSERT INTO filter_links (guild_id, domain, mode) VALUES (?, ?, ?)').run(interaction.guildId, domain, mode);
      ...
      if (sub === 'remove') {
        const domain = interaction.options.getString('domain').toLowerCase();
        const result = db.prepare('DELETE FROM filter_links WHERE guild_id = ? AND domain = ?').run(interaction.guildId, domain);
        if (result.changes === 0) return interaction.reply({ embeds: [errorEmbed('Not Found', 'Domain not in filters.')], ephemeral: true });
    ...
    if (sub === 'add') {
      ...
      try {
        db.prepare('INSERT INTO filter_words (guild_id, word, action) VALUES (?, ?, ?)').run(interaction.guildId, word, action);
    ...
    if (sub === 'remove') {
      const word = interaction.options.getString('word').toLowerCase();
      const result = db.prepare('DELETE FROM filter_words WHERE guild_id = ? AND word = ?').run(interaction.guildId, word);
      if (result.changes === 0) return interaction.reply({ embeds: [errorEmbed('Not Found', 'Word not in filters.')], ephemeral: true });
      return interaction.reply({ embeds: [successEmbed('Word Filter Removed', `\`${word}\` removed.`)], ephemeral: true });
    }

    if (sub === 'list') {
      await interaction.deferReply({ ephemeral: true });
      const words = db.prepare('SELECT * FROM filter_words WHERE guild_id = ?').all(interaction.guildId);
      const links = db.prepare('SELECT * FROM filter_links WHERE guild_id = ?').all(interaction.guildId);
